...
class ModelWiseTransformerClassifier(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        x = x.transpose(1, 2)
        seq_len = x.size(1)
        subseq_len = seq_len // 4
        # print("seq_len",seq_len,subseq_len)
        ...
        out4 = self.conv_feat_extract(x[:, 3:4, :])
        # print("原输出",(x).shape)
        # print("output层输出size",out1.shape,out2.shape,out3.shape,out4.shape)
        ...
        if self.training:
            ...
            paths, scores = self.crf.viterbi_decode(logits=logits, mask=mask)
            print(logits.shape)
            print(paths.shape[1],mask.shape)
            mask = mask[:, :paths.shape[1]]
            # max_seq_len = mask.sum(1)  # 计算实际的序列长度
            # mask = mask[:, :max_seq_len.max()] 
            paths[mask==0] = -1
            output = {'preds': paths, 'logits': logits}
            pass
