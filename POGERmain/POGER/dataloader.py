...
class POGERDataset(Dataset):
    def __init__(self, path, pretrain_model, max_len, k):
        ...
        self.k = k
        self.tokenizer = RobertaTokenizer.from_pretrained('../get_feature/roberta-base', local_files_only=True)
...
def get_dataloader(model_name, data_dir, pretrain_model, batch_size, max_len, k):
    if model_name == 'poger' or model_name == 'poger_wo_context':
        train_dataset = POGERDataset('../get_feature/train_poger_feature.jsonl', pretrain_model, max_len, k)
        test_dataset = POGERDataset('../get_feature/test_poger_feature.jsonl', pretrain_model, max_len, k)
...
def get_data(model_name, data_dir, pretrain_model, batch_size, max_len, k):
    train_dataset = POGERDataset('../get_feature/train_poger_feature.jsonl', pretrain_model, max_len, k)
    test_dataset = POGERDataset('../get_feature/detect_feature.jsonl', pretrain_model, max_len, k)
